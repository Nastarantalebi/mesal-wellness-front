...
const CalendarMatrixList = ({ data }: TProps) => {
  ...
  return (
    <div className="w-full overflow-x-auto rounded-xl border shadow bg-gray-100 dark:bg-gray-900">
      {/* Grid Header */}
      <div className="hidden sm:grid grid-cols-[150px_repeat(auto-fit,minmax(100px,1fr))] border-b border-gray-300 dark:border-gray-700">
        ...
        {data.dates.map((date, rowIndex) => (
          <div
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400 mb-2">
              {date} - تعداد: {data.rows.reduce((acc, r) => acc + r.days[rowIndex]?.count, 0)}
            </div>
            {data.rows.map((row: TRow) => {
              const day = row.days[rowIndex];
              return (
                <div key={row.id} className="mb-2">
                  <p className="font-bold text-gray-700 dark:text-gray-200">{row.name}</p>
                  {day.events.length === 0 && <div className="text-gray-400 text-xs">بدون رویداد</div>}
                  {day.events.map((event: TEvent) => (
                    ...
                    >
                      <p className="font-bold text-gray-800 dark:text-gray-100 truncate">{event.service}</p>
                      <p className="text-gray-500 dark:text-gray-400 truncate">{event.customer}</p>
