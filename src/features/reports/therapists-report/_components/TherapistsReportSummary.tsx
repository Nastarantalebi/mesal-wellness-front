...
const TherapistsReportSummary = ({ summary }: TProp) => {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
      <SammaryCard
        icon={<UserRoundSearch className="h-6 w-6" />}
        title="تعداد درمانگران"
        value={summary.total_therapists}
      />
      <SammaryCard
        icon={<Users className="h-6 w-6" />}
        title="درمانگران فعال"
        value={summary.active_therapists}
      />
      <SammaryCard
...
export default TherapistsReportSummary;
