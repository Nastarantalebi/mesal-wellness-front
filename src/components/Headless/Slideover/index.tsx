...
function Slideover({
  ...
  return (
    <slideoverContext.Provider value={{ open, zoom, size }}>
      <Transition appear as={Fragment} show={open}>
        <HeadlessDialog
          as={as}
          onClose={(value) => {
            if (!staticBackdrop) return onClose(value);
            setZoom(true);
...
// ✅ جدا تعریفش کن تا ESLint بفهمد کامپوننت است
function SlideoverPanel({
  children,
  className,
  as = "div",
  ...props
}: ExtractProps<typeof HeadlessDialog.Panel> & { size?: Size }) {
  ...
  return (
    <>
      ...
      <Transition.Child
        as="div"
        enter="ease-in-out duration-500"
        enterFrom="opacity-0 -me-[100%]"
        enterTo="opacity-100 me-0"
        leave="ease-in-out duration-[400ms]"
        leaveFrom="opacity-100 me-0"
        leaveTo="opacity-0 -me-[100%]"
        className="fixed inset-y-0 end-0"
      >
...
// بقیه‌ی زیربخش‌ها می‌مانند همانطور که نوشتی
